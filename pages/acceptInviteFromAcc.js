...
export async function init() {
    ...
    if (session.authenticated) {
        ...
        const res = await fetch('/db/acceptAccInvite', {method: 'POST', credentials: 'include', headers: {'Content-Type':'Application/json'}, body: JSON.stringify({ code: code })})
        ...
        const checkRes = await fetch('/db/checkInviteFromAcc', { method: 'POST', headers: {'Content-Type':'Application/json'}, body: JSON.stringify({ code: code })});
